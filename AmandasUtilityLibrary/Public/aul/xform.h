...
namespace aul
{
    ...
    struct xform
    {
        ...
        { }
        xform(const vector4<T>& x_, const vector4<T>& y_, const vector4<T>& z_, const vector4<T>& w_)
            : x(x_), y(y_), z(z_), w(w_)
        { }
        xform(const vector3<T>& diagonal, T w = scalar<T>::ONE)
        ...
        { }
        xform(const vector4<T>& diagonal)
            : m00(diagonal.x), m01(scalar<T>::ZERO), m02(scalar<T>::ZERO), m03(scalar<T>::ZERO)
            , m10(scalar<T>::ZERO), m11(diagonal.y), m12(scalar<T>::ZERO), m13(scalar<T>::ZERO)
            , m20(scalar<T>::ZERO), m21(scalar<T>::ZERO), m22(diagonal.z), m23(scalar<T>::ZERO)
        ...
        xform& operator=(const vector4<T>& diagonal);
        xform& operator=(const vector3<T>& diagonal);
        xform& operator=(const matrix4x4<T>& rhs);
        xform& operator=(const matrix3x3<T>& rhs);

        // xform operators
        inline bool operator==(const xform& rhs) const { return x == rhs.x && y == rhs.y && z == rhs.z && w == rhs.w; }
        inline bool operator!=(const xform& rhs) const { return x != rhs.x || y != rhs.y || z != rhs.z || w != rhs.w; }
        inline xform operator-() const { return xform(-x, -y, -z, -w); }
        inline xform operator+(const xform& rhs) const { return xform(x + rhs.x, y + rhs.y, z + rhs.z, w + rhs.w); }
        inline xform operator-(const xform& rhs) const { return xform(x - rhs.x, y - rhs.y, z - rhs.z, w - rhs.w); }
        inline xform operator*(T rhs) const { return xform(x * rhs, y * rhs, z * rhs, w * rhs); }
        inline friend xform operator*(T lhs, const xform<T>& rhs) { return xform<T>(lhs * rhs.x, lhs * rhs.y, lhs * rhs.z, lhs * rhs.w); }
        inline xform operator/(T rhs) const { return xform(x / rhs, y / rhs, z / rhs, w / rhs); }
        inline friend xform operator/(T lhs, const xform<T>& rhs) { return xform<T>(lhs / rhs.x, lhs / rhs.y, lhs / rhs.z, lhs * rhs.w); }
        ...
        inline xform& operator*=(T rhs) { x *= rhs; y *= rhs; z *= rhs; w *= rhs; return *this; }
        inline xform& operator/=(T rhs) { x /= rhs; y /= rhs; z /= rhs; w /= rhs; return *this; }
        ...
        inline bool operator!=(const matrix4x4<T>& rhs) const { return x != rhs.x || y != rhs.y || z != rhs.z || w != rhs.w; }
        inline matrix4x4<T> operator+(const matrix4x4<T>& rhs) const { return xform(x + rhs.x, y + rhs.y, z + rhs.z, w + rhs.w); }
        inline matrix4x4<T> operator-(const matrix4x4<T>& rhs) const { return xform(x - rhs.x, y - rhs.y, z - rhs.z, w - rhs.w); }
        matrix4x4<T> operator*(const matrix4x4<T>& rhs) const;

        inline vector4<T> major_diagonal() const { return vector4<T>(m00, m11, m22, m33); }
        inline xform& major_diagonal(T diagonal_x, T diagonal_y, T diagonal_z, T diagonal_w) { m00 = diagonal_x; m11 = diagonal_y; m22 = diagonal_z; m33 = diagonal_w; return *this; }
        inline xform& major_diagonal(const vector4<T>& diagonal) { m00 = diagonal.x; m11 = diagonal.y; m22 = diagonal.z; m33 = diagonal.w; return *this; }
        inline vector4<T> transform(const vector4<T>& point) const { return *this * point; }
        ...
        inline static xform make_uniform_scale(T scale) { return xform(vector4<T>(scale, scale, scale, scalar<T>::ONE)); }
        inline static xform make_scale(T scale_x, T scale_y, T scale_z) { return xform(vector4<T>(scale_x, scale_y, scale_z, scalar<T>::ONE)); }
        inline static xform make_scale(const vector3<T>& scale) { return xform(scale, scalar<T>::ONE); }
